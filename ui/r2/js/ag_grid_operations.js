...
var downloadGridOptions = {
    ...
    pagination: true,
    overlayLoadingTemplate: '<span style="padding: 10px; border: 2px solid #444; background: lightgoldenrodyellow;"> Logs / Markers / Trajectories will appear upon well selection for those well which have data</span>',
    columnDefs: [
        {
        ...
        {
            headerName: "File-ID", field: "fileName", width: 300, filterParams: {
...
const drawTable = async (data, gridOptionDetails) => {
    if (!gridOptionDetails)
        gridOptionDetails = downloadGridOptions
    if (data) {
...
function onSelectionChanged() {
    var selectedRows = uploaddownloadGridOptions.api.getSelectedRows();
    $('#Job-id-ingest').text("");
